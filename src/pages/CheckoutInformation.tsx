...
type DispatchInformationPayload = {
    mode: string
}
...
export const postInfoDetails = async (type = 'informationDetails', data: CheckoutInformationDetailsPayload | DispatchInformationPayload) => {
...
function CheckoutInformation() {
    ...
    useEffect(() => {
        ...
        if (informationDetails) {
            const { email, firstName, lastName, companyName, streetAndHouseNumber, pinCode, city, giftCard } = informationDetails;
    ...
    const [currentStage, setCurrentStage] = useState<CheckoutStatus>('Information');
        ...
        <div className='w-[100vw] h-[100vh] bg-[#f4f7f8] flex gap-[30px]'>
            <div className='ml-[50px] mt-[50px] w-[55vw] flex flex-col gap-[30px]'>
                ...
                <div className='flex items-center flex-wrap justify-start'>
                    ...
                    <div className='flex flex-col gap-[5px] items-center justify-center'
                        onClick={() => setCurrentStage('Information')}
                    >
                ...
                <div className='flex flex-col gap-[15px]'>
                    <div className='bebas desktop:text-[36px] mobile:text-[34px] uppercase'>Contact</div>
                    <input
                        type='email'
                        value={email}
                        onChange={(e) => setEmail(e.target.value)}
                        className={`rounded-full px-[30px] py-[10px]  ${hasErrors === 'email' && ' border-[1px] border-[#ea4c72]'}`}
                        placeholder="E-mail"
                    />
                </div>

                <div className='flex flex-col gap-[15px]'>
                    <div className='bebas desktop:text-[36px] mobile:text-[34px] uppercase'>Delivery Address</div>
                    <select name='country' className='px-[30px] py-[10px] text-[15px] rounded-full'>
                    ...
                    <div className='flex gap-[10px]'>
                        <input
                            ...
                            className='rounded-full px-[30px] text-[15px] py-[10px] w-[50%]'
                        ...
                        <input
                            ...
                            className={`rounded-full px-[30px] text-[15px] py-[10px] w-[50%] border-[1px] ${hasErrors === 'lastName' && 'border-[#ea4c72]'}`}
                    ...
                    <input
                        ...
                        className='rounded-full px-[30px] text-[15px] py-[10px]'
                    ...
                    <input
                        ...
                        className={`rounded-full px-[30px] text-[15px] py-[10px] ${hasErrors === 'streetAndHouseNumber' && 'border-[1px] border-[#ea4c72]'} `}
                    ...
                    <div className='flex gap-[15px]'>
                        <input
                            ...
                            className={`rounded-full px-[30px] text-[15px] w-[40%] py-[10px] ${hasErrors === 'pinCode' && 'border-[1px] border-[#ea4c72]'}`}
                            placeholder="Pincode"
                        />
                        <input
                            type='text'
                            ...
                            className={`rounded-full px-[30px] text-[15px] py-[10px] w-[60%] ${hasErrors === 'city' && 'border-[1px] border-[#ea4c72]'}`}
                            placeholder="City"
                        />
                    </div>
                    <button
                        onClick={() => {
                            if (email && lastName && streetAndHouseNumber && pinCode && city) {
                                postInfoDetails('informationDetails', {
                                    email: email,
                                    firstName: firstName,
                                    lastName: lastName,
                                ...
                                if (!email) setHasErrors('email')
                                else if (!lastName) setHasErrors('lastName')
                                else if (!streetAndHouseNumber) setHasErrors('streetAndHouseNumber')
                                else if (!pinCode) setHasErrors('pinCode')
                                else if (!city) setHasErrors('city')
                            }
                        }}
                        aria-describedby="button"
                        className={`relative blueButton btn justify-center flex items-center px-[10px] text-white uppercase tracking-wider bebas tracking-wider`}
